...
public void moveCoursesFromCartToUserCourse(UUID userID) {
    String insertQuery = "INSERT INTO UserCourse (userID, courseID) " +
                         "SELECT userID, courseID FROM Cart WHERE userID = ?";
    ...
    try (Connection con = DBConnection.getConnection()) {
        ...
        try (PreparedStatement psInsert = con.prepareStatement(insertQuery);
            ...
            psInsert.setString(1, userID.toString());
            psInsert.executeUpdate();

            psDelete.setString(1, userID.toString());
            psDelete.executeUpdate();

            con.commit(); // commit khi không lỗi
        } catch (SQLException e) {
            con.rollback(); // rollback nếu có lỗi trong insert/delete
            throw e; // ném lại để biết có lỗi
        }
