...
<c:if test="${not empty sessionScope.user}">
  ...
  <!-- Cửa sổ chat -->
  <div id="chatAIWindow" class="chat-ai-window" style="display:none;">
    <div class="chat-header" id="chatHeader">
      ...
      <div class="chat-header-actions">
        <button class="chat-action-btn" onclick="minimizeChat()" title="Thu nhỏ">
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none"><path d="M19 13H5V11H19V13Z" fill="currentColor"/></svg>
        </button>
    ...
    <div class="chat-input-area">
      <div class="chat-input-wrapper">
        <input id="chatInput" class="chat-input" type="text"
               placeholder="Nhập câu hỏi của bạn..." onkeypress="handleChatKeyPress(event)" />
        <button class="chat-send-btn" onclick="sendMessage()" title="Gửi">
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none">
  ...
  <!-- Cho JS biết contextPath -->
  <script> window.CHAT_AI_CONTEXT_PATH='${pageContext.request.contextPath}'; </script>

  <!-- Hoist 2 node ra body để tránh bị overlay cha che -->
  <script>
    (function(){
      const btn=document.getElementById('chatAIButton');
      const win=document.getElementById('chatAIWindow');
